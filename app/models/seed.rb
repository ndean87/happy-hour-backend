  ...
  def self.scrape
    ...
    entries.each do |entry|
      begin
        ...
        latitude = location.length && location[0].latitude
        longitude = location.length && location[0].longitude

        new_venue = Venue.create(venue_name: venue_name, address: venue_address, state: venue_state, city: venue_city, zipcode: venue_zipcode, phone_number: venue_phone, neighborhood: venue_neighborhood, latitude: latitude, longitude: longitude)
        ...
        specials.each do |special|
          ...
          new_venue.specials.create(day: day, special: deal, time: hours)
        end
      rescue
        puts 'error'
      end
